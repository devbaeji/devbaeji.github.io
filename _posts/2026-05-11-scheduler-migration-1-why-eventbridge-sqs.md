...
지금까지 살펴본 트리거 레이어와 발송 레이어를 한 장에 정리하면 이런 그림이 된다.

{% include scheduler-integrated-arch.html %}

위쪽(①) 은 *"5분마다 종 울려서 어떤 도메인 로직을 깨우기"*. 아래쪽(②) 은 *"깨어난 도메인이 결정한 알림을 외부로 fanout"*. 둘이 합쳐져서 *"AWS 매니지드 cron + 비동기 발송"* 한 줄짜리 설명의 풀스택 그림.

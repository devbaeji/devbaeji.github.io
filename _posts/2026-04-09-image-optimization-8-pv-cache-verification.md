...
브라우저 캐시를 완전히 배제하기 위해 DevTools에서 Disable cache를 끄고 확인했다.

![브라우저 캐시 비활성화](/assets/img/posts/image-optimization/disable-cache-off.png)

![네트워크탭 이미지 캐시 응답](/assets/img/posts/image-optimization/image-cache-network.png)

```bash
# 1회차 — MISS
curl -sI "https://test.example.com/stream-files/61?w=1024&q=90" | grep x-image-cache
# x-image-cache: MISS
...
![X-Image-Cache MISS](/assets/img/posts/image-optimization/x-image-cache-miss.png)

![X-Image-Cache HIT](/assets/img/posts/image-optimization/x-image-cache-hit.png)
...
```bash
kubectl -n develop rollout restart deploy/test-app-worker
kubectl -n develop rollout status deploy/test-app-worker
